...
class Scene_Zoom_In:
	def __init__(
		...
		**kwargs
		...
		self._image_clip=editor.ImageClip(
			self._image
			).set_position('center').resize(width=self._animation)
	...
	def _animation(self, time):
		if time==0:
			...
			return self._start_image_width+(time*((self._max_image_width-self._start_image_width)/(self._audio_clip.duration+self._end_margin)))
